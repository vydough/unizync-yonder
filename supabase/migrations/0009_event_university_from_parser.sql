-- UniVerse - use the parsed event university for club assignment
-- Run this after 0008_event_host_from_parser.sql.
...
begin
  ...
  select id into v_event_university_id
  from universities
  where lower(short_name) = lower(trim(p->>'university'))
     or lower(name) = lower(trim(p->>'university'))
  limit 1;

  -- Match the parsed host within the parsed university first. This prevents
  -- a same-named club at another campus from receiving the event.
  select id into v_club
  from clubs
  where lower(trim(name)) = lower(v_club_name)
    and (v_event_university_id is null or university_id = v_event_university_id)
  order by (university_id = coalesce(v_event_university_id, v_org.university_id)) desc, id
  limit 1;
  ...
  if v_club is null then
    ...
    values (v_club_name,
            ...
            coalesce(v_event_university_id, v_org.university_id))
