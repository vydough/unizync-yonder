...
const EVENT_FIELDS = `
  id, title, description, starts_at, ends_at, submitted_by,
  image_url,
...
/**
 * Where the saved event's primary action sends someone:
 *   1. page_url       -> the exact event page the organiser pasted
 *   2. ticket_url     -> a provider checkout only when no page exists
 *   3. the union      -> last resort for legacy catalogue rows
 * Every event page is a valid destination, including free events. A
 * student should land on the event they saved, never a provider homepage.
 */
export function destinationFor(event) {
  const uni = event.club.university;
  const label = event.price_cents === 0 ? 'Register' : 'Buy ticket';
  if (event.page_url)   return { kind: 'event', label, url: event.page_url, uni };
  if (event.ticket_url) return { kind: 'event', label, url: event.ticket_url, uni };
