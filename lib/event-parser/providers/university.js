...
/** UMSU/MSL event detail pages use labelled detail rows. */
function mslEventDetail(html) {
  const grab = selector => {
    const re = new RegExp(
      '<div[^>]+class=["\\\'][^"\\\']*' + selector + '[^"\\\']*["\\\'][\\s\\S]*?' +
      '<span[^>]+class=["\\\'][^"\\\']*ev-details[^"\\\']*["\\\'][^>]*>([\\s\\S]*?)<\\/span>', 'i'
    );
    const match = html.match(re);
    return match ? clean(match[1], 300) : '';
  };

  const titles = [...html.matchAll(/<h1[^>]*>([\s\S]*?)<\/h1>/gi)]
    .map(match => clean(match[1], 140))
    .filter(value => value && value.toLowerCase() !== 'events');
  const title = titles.at(-1) || '';
  if (!title) return null;

  const dateText = grab('details-date');
  const timeText = grab('details-time');
  ...
  const toIso = (day, month, year, hour, minute, meridiem) => {
    const values = [day, month, year, hour, minute].map(Number);
    if (values.slice(0, 4).some(Number.isNaN)) return '';
    const [d, mo, y, rawHour, rawMinute] = values;
    let h = rawHour;
    if (/pm/i.test(meridiem) && h !== 12) h += 12;
    if (/am/i.test(meridiem) && h === 12) h = 0;
    const date = new Date(`${y}-${String(mo).padStart(2, '0')}-${String(d).padStart(2, '0')}T${String(h).padStart(2, '0')}:${String(rawMinute || 0).padStart(2, '0')}:00+10:00`);
    return isNaN(date.getTime()) ? '' : date.toISOString();
  };

  const out = { title };
  if (dateMatch) {
    const startHour = timeMatch?.[1] || 0;
    const startMinute = timeMatch?.[2] || 0;
    const startMeridiem = timeMatch?.[3] || 'AM';
    out.startDate = toIso(dateMatch[1], dateMatch[2], dateMatch[3], startHour, startMinute, startMeridiem);
    if (dateMatch[4]) {
      out.endDate = toIso(dateMatch[4], dateMatch[5], dateMatch[6], endTimeMatch?.[1] || startHour, endTimeMatch?.[2] || startMinute, endTimeMatch?.[3] || startMeridiem);
    }
  }
  const venue = grab('details-location');
  const organiser = grab('details-org');
  if (venue) out.venue = { name: venue };
  if (organiser) out.organiser = organiser;

  const image = html.match(/<div[^>]+class=["'][^"']*contentWrapper[^"']*["'][\s\S]*?<img[^>]+src=["']([^"']+)["']/i);
  if (image) out.imageUrl = new URL(decodeEntities(image[1]), 'https://umsu.unimelb.edu.au').href;
  const description = html.match(/<div[^>]+class=["'][^"']*details-org[^"']*["'][\s\S]*?<\/div>([\s\S]*?)(?=<div[^>]+class=["'][^"']*e-ticket[^"']*["'])/i);
  if (description) out.description = clean(description[1], 1500);
  return out;
}
...
export const university = {
  ...
  parse(url, html) {
    const out = liveWhale(html) || mslEventDetail(html) || msl(html) || {};
    ...
    out.ticketUrlFallback = out.pageUrl;   // most union events are free
    return out;
  }
};
