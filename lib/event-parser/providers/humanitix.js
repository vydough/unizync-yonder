...
/** Read the fields Humanitix renders outside its JSON hydration payload. */
function renderedFields(html) {
  const host = html.match(/<div[^>]+class=["'][^"']*\bhostname\b[^"']*["'][^>]*>[\s\S]*?<a[^>]*>([\s\S]*?)<\/a>/i);
  const description = html.match(/<div[^>]+class=["'][^"']*\bRichContent\b[^"']*["'][^>]*>([\s\S]*?)<\/div>/i);
  return {
    organiser: host ? clean(host[1], 90) : '',
    description: description ? clean(description[1], 1500, true) : ''
  };
}
...
export const humanitix = {
  ...
  parse(url, html) {
    ...
    const rendered = renderedFields(html);
    ...
    if (rendered.description) out.description = rendered.description;
    if (rendered.organiser) out.organiser = rendered.organiser;
