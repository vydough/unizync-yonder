/* ================================================================
   ...
   merge (last wins, blanks skipped) → normalise → preview → save
   ================================================================ */
 ...
 */
function mergeInto(into, patch, sourceName) {
...
const humanitix = {
  ...
  parse(url, html) {
    ...
    // Humanitix checkout is always the event page plus /tickets, which
    // saves a student one tap. It's still a derivation, so it goes in
    // ticketUrlFallback: a checkout link the page declares itself wins.
    const base = url.href.split('?')[0].replace(/\/+$/, '');
    if (!/\/tickets$/.test(base)) out.ticketUrlFallback = base + '/tickets';
    out.pageUrl = base;
    return out;
  }
};
...
const rubric = {
  ...
  parse(url, html) {
    ...
    out.ticketUrlFallback = out.pageUrl;   // Rubric has no separate checkout
    return out;
  }
};
...
function parseEventFromHtml(text, sourceUrl) {
  ...
  // which wins because they run after this.
  const firstUrl = sourceUrl || (raw.match(/https:\/\/[^\s"'<>]+/) || [])[0] || '';
  const cleanFirst = firstUrl.replace(/[).,;'"]+$/, '');
  if (cleanFirst) {
    // ticketUrl goes in as a fallback: if the page turns out to declare
    // a real checkout link, or a provider knows where its checkout
    // lives, that beats "wherever they pasted from".
    mergeInto(event, { pageUrl: cleanFirst, ticketUrlFallback: cleanFirst }, 'url-only');
  }
  ...
  if (looksLikeHtml) {
    // Worst first, best last. The merger's rule is "last one wins,
    // and a blank never overwrites", so this order IS the priority.
    ...
    // Provider-specific last, so it wins — but only for the fields it
    ...
    if (url) {
      ...
      if (provider) {
        try {
          mergeInto(event, provider.parse(url, raw), provider.name);
...
/**
 * Everything downstream reads this shape and nothing else. Also
 * attaches what the preview screen needs: which parser gave us each
 * field, and which fields a human still has to fill in.
 */
function normalise(event, sourceUrl) {
  ...
  const out = {
    ...
    // A checkout link the page declared beats one a provider derived.
    ticketUrl:   event.ticketUrl || event.ticketUrlFallback || '',
    source: {
