 ...
 *  come out one way. */
function clean(s, max = 1500, preserveNewlines = false) {
  return decodeEntities(String(s == null ? '' : s))
    .replace(/<br\s*\/?>/gi, preserveNewlines ? '\n' : ' ')
    .replace(/<\/(p|div|li|h[1-6])[^>]*>/gi, preserveNewlines ? '\n' : ' ')  // block ends
    .replace(/<[^>]+>/g, '')                                 // inline tags leave nothing,
                                                             // so "<b>ink</b>." stays "ink."
    .replace(preserveNewlines ? /[ \t]+/g : /\s+/g, preserveNewlines ? ' ' : ' ')
    .replace(preserveNewlines ? /\n[ \t]+/g : /$^/, preserveNewlines ? '\n' : '')
    .replace(preserveNewlines ? /\n{3,}/g : /$^/, preserveNewlines ? '\n\n' : '')
