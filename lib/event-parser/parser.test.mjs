...
import { mergeDefined, overallConfidence } from './mergeDefined.js';
...
eq('og:image is the source',   r.provenance.imageUrl.source, 'opengraph');
...
/* ============================================================
   3. Merge order and confidence
   ============================================================ */
section('Merging by confidence');
...
eq('and each field records its source', r.provenance.title.source, 'json-ld');
eq('image kept its own source',         r.provenance.imageUrl.source, 'opengraph');

const merged = mergeDefined(
  [{ title: 'low', venue: { name: 'Generic Hall' } }, 'generic-html'],
  [{ title: 'high', venue: { address: '1 Test St' } }, 'json-ld']
);
eq('higher confidence wins',   merged.title, 'high');
eq('venue merges key by key',  merged.venue, { name: 'Generic Hall', address: '1 Test St' });
ok('overall confidence is 0–1', overallConfidence(merged) > 0 && overallConfidence(merged) <= 1);
...
eq('humanitix: checkout url',             r.ticketUrl, 'https://events.humanitix.com/rooftop-screening/tickets');
...
ok('confidence is reported',   r.confidence >= 0 && r.confidence <= 1, String(r.confidence));
